...
def remap_image(name, img, small, page_dims, params):
    ...
    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='_thresh.png')
...
def page_dewarp(dewarped_list, image_list):
    ...
    for index, imgfile in enumerate(dewarped_list):
        img = imgfile
        small = resize_to_screen(img)
        basename = os.path.basename(image_list[index])
